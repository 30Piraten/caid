...
resource "aws_iam_role" "ec2_instance_role" {
  ...
  permissions_boundary = var.permissions_boundary_arn # why null value?

  tags = {
    Name        = "ec2-scheduler-role"
    Environment = var.env
    Purpose     = "EC2 Instance Scheduling"
  }
}
...
*/
resource "aws_iam_policy" "ec2_optimizer_policy" {
  name        = "ec2-cost-optimization-policy" # Fixed typo in name
...
resource "aws_iam_role_policy_attachment" "ec2_policy_attachment" {
  policy_arn = aws_iam_policy.ec2_optimizer_policy.arn
  role       = aws_iam_role.ec2_instance_role.name 
}
